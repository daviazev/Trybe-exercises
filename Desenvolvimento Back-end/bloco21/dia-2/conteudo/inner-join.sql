...
-- aqui tinha uma imagem ilustrando o INNER JOIN

/*
	Por que usamos o alias (AS)?
	O alias AS é usado para apelidar qualquer parte da sua query. 
    Isso ajuda o sistema de banco de dados a identificar a qual coluna 
    estamos nos referindo, evitando erros de ambiguidade de nome de 
    colunas, além de tornar suas queries mais concisas e legíveis.
...
/*
	O código acima, além de ser muito extenso, não permite que o 
	banco de dados descubra de qual tabela deve trazer o actor_id, 
	uma vez que ambas as tabelas, actor e filme_actor, possuem uma 
	coluna actor_id.
*/
...
SELECT xablau.first_name, xablau.actor_id, xablou.actor_id
FROM sakila.actor xablau
INNER JOIN sakila.film_actor xablou
ON xablau.actor_id = xablou.actor_id;

-- PARA FIXAR -- AINDA A SER FEITO!!!
